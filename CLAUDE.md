...
Phase 7a — safety/moderation (done). Rate-limit (`apps/api/src/common/rate-limit.ts` mixin guard + WS check), reports (`/reports` user, `/admin/reports[/:id/resolve]` operator with `AdminGuard` reading `env.ADMIN_TOKEN`), blocks (`@Global` `BlocksService.existsBetween` enforced in discover/matches/swipes/chat), soft-delete (`User.deletedAt`/`bannedAt`/`banReason`, JwtAuthGuard re-checks every request, returns `403 { code }`). `apps/web/src/admin/AdminScreen.tsx` opens via `?admin=<ADMIN_TOKEN>` and short-circuits the user flow. Anti-deanon `*.spec.ts` extended with bypass-fixation tests (cyrillic, full-width, ZWJ).

Phase 7b — realtime polish (done). `Message.editedAt`/`readAt`, `Match.archivedByA/B`, `User.lastSeenAt`. WS `message:edit` (15-min own-only window, 410 GONE outside), `message:read` (one batched updateMany + single `MessageReadPayload` event with `messageIds[]`), `chat:typing` (no DB; broadcast minus sender; 5/5s server throttle on top of 3s client debounce). Presence: `markOnline`/`markOffline` return transitions; `touchAndAnnouncePresence` stamps `lastSeenAt` and fans out `user:presence` to chat partners via user-rooms. REST `GET /users/:userId/presence` (assertSharedChat → 403 for strangers). `GET /chats/:id/messages?after=ISO` (ASC, no pagination) for reconnect-resync. `Match` archive endpoints + `?archived` filter on `/matches`. Frontend: `useChat` exposes `loadMore/editing/markRead/sendTyping/typingUntil`, IntersectionObserver-based infinite scroll up that preserves anchor, long-press bubble menu (Edit/Copy), ✓/✓✓ ticks, "(изм.)" marker, header subtitle priority typing > online > "был N мин назад", Active/Archive tabs in `MatchesList`.

Phase 7c — UX, discovery, sharing (done). `NotificationPrefs` model (matches/messages/digestMode/mutedUntil) + `/me/notifications` GET/PATCH. `NotificationsService` honours prefs with a 60s in-memory cache + 10-min digest flush loop for `digestMode` users. Discover filters: `?verticals=&geos=` CSV, intersect with profile compat, empty intersection short-circuits to no candidates rather than ignoring the filter. `GET /me/likes/count` returns inbound LIKEs without revealing who (paid-reveal hook later). `DELETE /swipes/last` undoes the most recent swipe within a 60s window; refuses (409 MATCH_EXISTS) if the swipe already produced a Match. Referrals: `User.referredById` + `User.pendingViewProfile`; bot's `/start` handler (`apps/bot/src/start-payload.ts`, uses `@prisma/client` directly) writes deep-link payloads (`p_<id>`, `ref_<id>`) to the User row, creating a stub if needed; auth.service merges on first login (referredById is write-once, never updated). `GET /users/:userId/card` returns any non-deleted/non-banned/non-blocked user's PublicCard. `GET /me/referral-link` builds `t.me/<botUsername>?start=ref_<myId>` (bot username cached in-process). `MeResponse` extends PublicUser with `referralCount` + `pendingViewProfile`. Frontend: drag-to-swipe via `framer-motion` (`motion.div` with `drag="x"`, like/skip overlay opacity tracking dx, threshold 100px). `FilterSheet` chip-picker, persisted via `useLocalStorageState`. `useLikesCount` polls `/me/likes/count` once a minute + on focus, drives the Матчи tab badge AND a banner inside `MatchesList`. `Deck` shows a 5s undo toast after every non-match swipe → `DELETE /swipes/last`. `UserCardScreen` overlay opens from `getStartParam()` (`?startapp=p_<id>`) OR from `MeResponse.pendingViewProfile` (bot side); supports Like/Skip in-place and clears `pendingViewProfile` server-side after consumption. `SettingsScreen` adds notification toggles + mute-presets (1h/8h/until-morning). `MyProfile` adds a referral card with copy + Telegram share-sheet (`shareLink` helper).

Phase 7d — deploy + full admin (current). README, prod Dockerfiles. **Admin console** is offered in two surfaces: (1) a web operator screen at `?admin=<ADMIN_TOKEN>` (`apps/web/src/admin/`), (2) `/admin` inline-keyboard menu inside the Telegram bot (`apps/bot/src/admin.ts`). Both go through the same `apps/api/src/admin/` HTTP endpoints over `AdminGuard`. Bot whitelisting via `env.ADMIN_TELEGRAM_IDS` (CSV of telegram ids); the bot also sets `setMyCommands` per-chat scope to expose `/admin` in the autocomplete only to whitelisted users. New endpoints: `GET /admin/stats`, `GET /admin/users?q&status&role&take&skip`, `GET /admin/users/:id`, `POST /admin/users/:id/ban|unban|reset-role`, `GET /admin/chats/:id/messages` (full transcript with sender anonIds for forensics — content is the antiDeanon-scrubbed value already in DB). Frontend uses inline operator-style dark UI in `apps/web/src/admin/admin-styles.tsx` — deliberately separate from the Tailwind brand tokens so admin doesn't pick up TG theme overrides. `ChatGateway.onlineCount()` returns the distinct-online-users count for stats.

**Role-reset** (user-facing): `DELETE /onboarding/role` clears `User.role` + `anonId` + deletes the per-role profile row in a single transaction. Existing matches/swipes/messages stay intact — the user simply lands back in `RolePicker` on next render. Settings exposes the action under "Аккаунт → Сменить роль". After re-onboarding the user gets a NEW anonId from `AnonCounter` (counter never decrements), and chat partners will see the new anonId on subsequent reads of all messages — anonId is joined fresh from `User` per fetch (no per-message snapshot, intentional). Admin has the equivalent `POST /admin/users/:id/reset-role` for clearing on behalf of a user.
